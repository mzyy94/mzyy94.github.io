...
CECのデータは送信元4ビット、宛先4ビット、コード8ビット、それに続いて引数がつく形で表されます。
この0f:36は次のように分解できます。
...
送り方がわかると、CECデータを生で送って調査したくなりますね。
**10:04**を送ってみました。
...
CECの調査では、デバイスと連動してスリープにちゃんと移行しない場合など、どのようなCECデータがやり取りされているかを確認できます。
また、どのようなデータを送るとどういった挙動をするのか、動作確認にも使えますね。
